...
export interface TableStore {
  ...
  getNote?: (categoryId: string, monthIndex: number) => string;
...
function getDueClass(
  ...
  if (!cat.dueDay || status !== "unpaid" || currentMonth !== mi)
    return undefined;
...
export default function ExpenseTable({ store, filter, editMode }: Props) {
  ...
  const currentMonth = useMemo(
    () => getCurrentMonthIndex(data.year),
    [data.year],
  );
  ...
  function handleCellClick(
    ...
    // Szybki tryb z górnego filtra zostaje: wybierz M/J/M+J i klikaj puste komórki.
    if (filter !== "all" && status === "unpaid") {
      setStatus(cat.id, mi, smartPaidStatus(filter));
      return;
    }

    // W trybie Wszystkie kliknięcie komórki otwiera menu szybkiej edycji.
    setQuickEdit({
      cat,
      monthIndex: mi,
  ...
  return (
    <>
        ...
        >
          <table className="border-collapse text-sm w-full table-fixed">
            <colgroup>
              ...
              {cats.map((cat) => (
                <col key={cat.id} style={{ minWidth: CAT_COL_MIN }} />
              ))}
            </colgroup>
                ...
                {cats.map((cat) => {
                  ...
                  return (
                    <th
                      key={cat.id}
                      onMouseEnter={() =>
                        setHover((h) => ({ row: h?.row ?? -1, col: cat.id }))
                      }
                      onMouseLeave={() => setHover(null)}
                      style={
                        cat.color ? { borderTop: `3px solid ${cat.color}` } : {}
                      }
                    ...
                    >
                      <span
                        className="text-[11px] font-semibold text-foreground leading-tight flex items-center justify-center gap-1 truncate"
                        title={cat.name}
                      >
                        {cat.color && (
                          <span
                      ...
                      {cat.installmentMonths &&
                        (() => {
                          const paidN = data.cells.filter(
                            (c) =>
                              c.categoryId === cat.id && PAID.has(c.status),
                          ).length;
                          const total = cat.installmentMonths;
                          const done = paidN >= total;
                          const pct = Math.min(
                            100,
                            Math.round((paidN / total) * 100),
                          );
                          return (
                            <div className="mt-1 w-full px-1">
                              <div className="flex items-center justify-between mb-0.5">
                                <span
                                  className={`text-[8px] font-bold ${done ? "text-chart-3" : "text-primary"}`}
                                >
                                  {done
                                    ? "✓ spłacone"
                                    : `${paidN}/${total} rat`}
              ...
              {MONTHS.map((month, mi) => {
                ...
                return (
                  ...
                  >
                    {/* Month label */}
                    <td
                      className={`sticky left-0 z-10 py-2.5 px-3 text-xs font-medium border-r border-border/50 whitespace-nowrap transition-colors ${
                        isCurrent
                      ...
                      return (
                        ...
                        >
                          <span className="text-xs flex items-center justify-center gap-0.5 w-full">
                            {CELL_LABEL[status]}
                            {dueClass && (
                              <span className="absolute top-1 right-1 h-1.5 w-1.5 rounded-full due-dot" />
                            )}
                            {showLock && (
                              <Lock className="h-2 w-2 opacity-20 shrink-0" />
                            )}
                            {(() => {
                              const n = data.cells.find(
                                (c) =>
                                  c.categoryId === cat.id &&
                                  c.monthIndex === mi,
                              )?.note;
              ...
              <tr className="bg-muted/90 backdrop-blur-sm border-t border-border print:bg-gray-100">
                ...
                {cats.map((cat) => {
                  ...
                  return (
                    <td
                      key={cat.id}
                      className="border-l border-border/30 py-2 text-center"
                    >
                      <span
                        className={`text-[10px] font-semibold ${paid === req && req > 0 ? "text-emerald-600 dark:text-emerald-400" : "text-muted-foreground"}`}
                      >
                        {paid}/{req}
                      </span>
                    </td>
      ...
      {quickEdit && (
        <CellEditPopover
          catName={quickEdit.cat.name}
          month={MONTHS[quickEdit.monthIndex]}
          status={getStatus(quickEdit.cat.id, quickEdit.monthIndex)}
          note={store.getNote?.(quickEdit.cat.id, quickEdit.monthIndex) ?? ""}
          names={names}
          anchorRect={quickEdit.rect}
          onClose={() => setQuickEdit(null)}
          onSet={(status) =>
            setStatus(quickEdit.cat.id, quickEdit.monthIndex, status)
          }
          onSetNote={(note) =>
            store.setNote?.(quickEdit.cat.id, quickEdit.monthIndex, note)
          }
        />
      )}
    </>
