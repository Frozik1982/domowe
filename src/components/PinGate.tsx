...
export default function PinGate({ children }: { children: React.ReactNode }) {
  ...
  // Utrzymuje fokus na ukrytym polu — szybciej działa klawiatura numeryczna i wklejanie PIN-u.
  useEffect(() => {
    if (!authorized) {
    ...
    >
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        <div
          ...
          style={{
            background: `radial-gradient(circle, hsl(var(--chart-2) / 0.07), transparent)`,
          }}
        />
      ...
      <div
        className={`relative w-full max-w-xs mx-4 ${shake ? "animate-shake" : ""}`}
      >
        <div className="bg-card/95 backdrop-blur-xl border border-border/60 rounded-3xl shadow-2xl overflow-hidden">
          ...
          <div className="p-8 text-center">
            ...
            <p className="text-[10px] text-muted-foreground/45 mt-5">
              Możesz wpisać PIN z klawiatury, wkleić go albo użyć przycisków.
            </p>
          </div>
        </div>
      </div>
